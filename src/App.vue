...
export default {
  ...
  created () {
    ...
    if (that.isWeiXin) {
      if (that.getQueryString('token') || that.getLocalObj('user')) {
        if (that.getQueryString('token')) {
          console.log('链接后面的token ' + that.getQueryString('token'))
          console.log('把链接token放入本地存储')
          ...
          if (nowDate - that.getLocalObj('user').token_time >= that.$store.state.check_time * 1000) {
            console.log('时间过期')
            console.log('本地user ' + window.localStorage.getItem('user'))
            console.log('本地user ' + window.location.href)
