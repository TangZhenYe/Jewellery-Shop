<template>
	<div class="box cart_editAddress">
		<div class="list">
			<div class="per-list">
...
export default {
	...
	created () {
		let that = this
		that.$store.commit('updateTitleMsg', { title_msg: '修改地址', })
		that.is_receive = this.$route.query.is_receive
		that.address_id = this.$route.query.address_id
		if (!that.getLocalObj('addressList_' + that.address_id).provinceLists) {
			that.getProvinceLists()
		} else {
			that.provinceLists = that.getLocalObj('addressList_' + that.address_id).provinceLists
		}
		that.c_ajax({address_id: that.address_id, is_receive: that.is_receive}, 'api.php?c=UserAddress&a=detail', function (value) {
			if (value.data.status === 10001) {
				that.phone = value.data.detail.phone
				that.name = value.data.detail.name
				that.address = value.data.detail.details_site
				that.province_id = value.data.detail.province
				that.checkPicture = (value.data.detail.type === '0') ? false : true
				if (!that.getLocalObj('addressList_' + that.address_id).cityLists) {
					that.getCityLists(that.province_id)
				} else {
					that.cityLists = that.getLocalObj('addressList_' + that.address_id).cityLists
				}
				that.city_id = value.data.detail.city
				setTimeout(function() {
					if (!that.getLocalObj('addressList_' + that.address_id).areaLists) {
						that.getAreaLists(that.city_id)
					} else {
						that.areaLists = that.getLocalObj('addressList_' + that.address_id).areaLists
					}
